...
if(isset($_POST['buttonCourseHot'])){
	$getIdBreadcrumb = $_POST['breadcrumb'];
	$mergeCourse = "SELECT courses.*, departments.name as departments_name, users.name = users_name 
	FROM `courses`JOIN departments ON departments.id = courses.department_id AND departments.fuculty_id = $getIdBreadcrumb
	JOIN users ON courses.teacher_id = users.id ORDER BY teacher_id ASC LIMIT 4";
		echo "<pre>";
		print_r($mergeCourse);
		echo "</pre>";
		die();
	// $query_mergeCourse = mysqli_query(openConnection(),$mergeCourse);
	// $array_mergeCourse = mysqli_fetch_array($query_mergeCourse);
	...
	<main class="min-vh-100">
				...
				<div class="row">
					<ul class="list-unstyled d-flex pt-3 pb-3 mb-0">
						...
						?>
							...
							class="bi bi-caret-right-fill" style="font-size: 12px;"></i><?php echo $arrayBreadcrumbAndChild['faculties_name'] ?></a></li>
							<li><a href="#" class="text-decoration-none text-black fw-bold" style="font-size: 16px;"><i
							class="bi bi-caret-right-fill" style="font-size: 12px;"></i><?php echo $arrayBreadcrumbAndChild['name'] ?></a></li>
						<?php 
						} elseif (isset($_GET['breadcrumb'])) {
						
						?>
							<li><a href="#" class="text-decoration-none text-black fw-bold" style="font-size: 16px;"><i
							class="bi bi-caret-right-fill" style="font-size: 12px;"></i><?php echo $array_Breadcrumbt['name'] ?></a></li>
						<?php 
						}
						?>
					</ul>
		...
		<div class="container-fluid" style="background-color: #f5f5f5;">
			...
			<div class="container">
					...
					<div class="col-12">
						<div class="mt-3 mb-2">
								<?php 
									if(isset($_GET['breadcrumb']) && isset($_GET['breadcrumbChild'])){
								?>
										<p class="fw-bold" style="font-size: 22px;">Course <?php echo $arrayBreadcrumbAndChild['name'] ?></p>
								<?php 
									}else if (isset($_GET['breadcrumb'])) {
								?>
									<p class="fw-bold" style="font-size: 22px;">Course <?php echo $array_Breadcrumbt['name'] ?></p>
								<?php 
									} /*else{
				...
				<div class="row">
					<form action="pagesubject.php" method="post">
						<ul class="d-flex list-unstyled nav-tab">
							<button name="buttonCourseHot" class="button active" style=" border: none; padding: 10px 15px; background: #fff; border-radius: 4px; margin-right: 2px;">
								<li>
									<a href="#" class="text-decoration-none border-0" style="color: #555555">Khoá học phổ biến</a>
								</li>
							</button>

							<button name="buttonCourseNew" style=" border: none; padding: 10px 15px; background: #fff; border-radius: 4px; margin-right: 2px;">
								<li>
									<a href="#" class="text-decoration-none border-0" style="color: #555555">Khoá học mới</a>
								</li>
							</button>
						</ul>
					</form>
							...
							<swiper-slide>
								<a class="text-decoration-none text-black position-relative" href="#">
									<div class="card shadow bg-body rounded">
										<img src="../public/images/khoa-hoc-dem-hat-guitar-co-ban.jpeg"
				...
				<div class="row blog-content">
					<div class="col-12">
						<p>Trong cuộc sống hiện nay, không thể phủ nhận tầm quan trọng của việc học ngoại ngữ. 
							Việc thông thạo ngoại ngữ sẽ giúp cho bạn mở ra nhiều cơ hội lớn trong học tập cũng như công việc. </p>
						<p>Xuất phát từ điều này, UNICA đã kết hợp với các giảng viên giàu chuyên môn xây dựng nên những khóa học ngoại ngữ online cực kỳ hữu ích, 
							đáp ứng tối đa mọi nhu cầu học tập của học viên. </p>
						<h2>
							<strong>Sở hữu 145 khóa học ngoại ngữ ưu việt </strong>
						</h2>
						<p>
							Khi nhắc đến khóa học ngoại ngữ, nhiều học viên sẽ nghĩ rằng chỉ để cập đến bộ môn tiếng Anh là chủ yếu. 
							Tuy nhiên, đến với UNICA, bạn còn được nhiều hơn thế, bởi UNICA không chỉ cung cấp các khóa học tiếng Anh 
							mà còn có 3 ngoại ngữ đang rất “thịnh hành” nhất hiện nay là tiếng Hàn, tiếng Nhật và tiếng Trung. 
							Điều này sẽ giúp “đánh bay” nỗi lo trong việc chọn khóa học phù hợp với ngôn ngữ mà mình yêu thích. 
						</p>
						<p>
							Với 4 ngoại ngữ chính sẽ được chia thành từng nhóm kiến thức và trình độ khác nhau. Ví dụ như: cơ bản và nâng cao, 
							giao tiếp, ngữ pháp, luyện thi… Như vậy, các học viên có thể lựa chọn được khóa học phù hợp với năng lực và mục tiêu 
						...
						<p>
							Ưu điểm lớn nhất mà các khóa học ngoại ngữ online trên UNICA mang lại đó chính là bạn có thể học mọi lúc mọi nơi. 
							Do tính chất học online nên học viên có thể chủ động trong việc học mà không bị gò bó về thời gian. 
						</p>

						<p>
							Chính điều này sẽ giúp cho bạn tiết kiệm thời gian học và mang lại hiệu quả cao nhất trong quá trình tiếp nhận kiến thức. 
							Đặc biệt, chi phí mà bạn cần bỏ ra cho một khóa học rẻ hơn gấp nhiều lần so với các trung tâm ngoại ngữ. Như vậy, 
						...
						<p>
							Đến với khóa học ngoại ngữ trên UNICA, bạn sẽ được “cầm cân chỉ mực” bởi những giảng viên giàu chuyên môn như: 
							Th.s Ruby Thảo Trần, Victoria Dương, Hannah Phạm, Thanh Trúc, Vũ Thùy Linh, EZ Language, Ngô Trần Minh Thảo… 
						</p>
						<p>Các giảng viên tham gia giảng dạy ngoại ngữ trên UNICA đều đạt các chứng chỉ xuất sắc của các cuộc thi trong nước 
							và quốc tế. Đặc biệt, cách thức giảng dạy, trình bày và chia sẻ kiến thức của các giảng viên đều rất khoa học, dễ hiểu và 
							cực kỳ thú vị. Do đó, bạn sẽ không phải quá lo lắng về việc sẽ-ngủ-gật-trong-giờ-học.
						</p>

						<p>
							Nếu việc tự học tại nhà qua sách vở, học qua trung tâm, trường lớp sẽ khiến bạn phải mất quá nhiều thời gian mới có thể tiếp nhận kiến thức, 
							thì UNICA sẽ giúp bạn khắc phục hoàn toàn vấn đề này. 
						</p>

						<p>
							Với 3 lợi ích siêu ưu việt mà các khóa học ngoại ngữ trên UNICA cung cấp cho bạn, chắc chắn sẽ giúp việc chạm đến ước mơ, mục tiêu của bạn 
