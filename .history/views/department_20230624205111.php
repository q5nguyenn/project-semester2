...
if (isset($_GET['id'])) {
	...
	$selector_Departments = "SELECT  departments.*, faculties.name as faculties_name FROM `departments` JOIN faculties ON departments.faculty_id = faculties.id WHERE departments.id = '$getIdDepartments'";
...
if (isset($_GET['filter'])&& $_GET['filter'] == 'morestudy') {
	...
	FROM `courses` JOIN  departments ON courses.department_id = departments.id AND departments.id = '$getIdDepartments' 
	JOIN users ON courses.teacher_id = users.id 
	JOIN reviews ON courses.id = reviews.course_id
	GROUP BY id 
	HAVING AVG(rate) > 4
					...
					<div class="col-8">
						<div class="d-flex align-items-center u-hot-cate justify-content-between">
							...
							<form action="" method="post">
								...
								<li><a href="department.php?id=<?php echo $array_Departments['id']?>&filter=morestudy" class="text-decoration-none">Học nhiều nhất</a></li>
								<li><a href="department.php?id=<?php echo $array_Departments['id']?>&filter=highvote" class="text-decoration-none">Đánh giá cao</a></li>
								<li><a href="department.php?id=<?php echo $array_Departments['id']?>&filter=newStudy" class="text-decoration-none">Mới nhất</a></li>
								<li><a href="department.php?id=<?php echo $array_Departments['id']?>&filter=pricelowertohigh" class="text-decoration-none">Giá thấp đến giá cao</a></li>
								<li><a href="department.php?id=<?php echo $array_Departments['id']?>&filter=pricehightolower" class="text-decoration-none">Giá cao đến giá thấp</a></li>
								</ul>
							</form>
							<form action="department.php?id=<?php echo $array_Departments['id']?>" method="post">
