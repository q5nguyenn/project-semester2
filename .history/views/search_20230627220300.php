<?php 
	...
	if ($query_searchProduct) {
		...
		if (isset($_GET['keyword']) && isset($_GET['acset'])) {
			$searchProduct = "SELECT courses.*, departments.name as departments_name, users.name as users_name, users.thumbnail as users_thumbnail 
			FROM `courses` JOIN departments ON courses.department_id = departments.id  JOIN users ON courses.teacher_id = users.id WHERE  courses.name LIKE '%".$keyword."%' ORDER BY teacher_id ASC ";
			$query_searchProduct = mysqli_query(openConnection(), $searchProduct);
							...
							<div class="d-flex align-items-center u-hot-cate shadow-sm p-3 mb-5 bg-body rounded" style="background-color: #fff;">
								...
								<form action="" method="post">
									<ul class="d-flex list-unstyled m-0">
										<button name="studymore" class="p-0 btn background-none"><li><a href="searchController.php?keyword=<?php echo $keyword ?>&acset=morestudy" class="text-decoration-none">Học nhiều nhất</a></li></button>
										<button name="votehigh" class="p-0 btn background-none"><li><a href="searchController.php?keyword=<?php echo $keyword ?>&acset=votehight" class="text-decoration-none">Đánh giá cao</a></li></button>
										<button name="coursenew" class="p-0 btn background-none"><li><a href="searchController.php?keyword=<?php echo $keyword ?>&acset=studynew" class="text-decoration-none">Mới nhất</a></li></button>
										<button name="pricelowertohigh" class="p-0 btn background-none"><li><a href="searchController.php?keyword=<?php echo $keyword ?>&acset=pricelowtohight" class="text-decoration-none">Giá thấp đến giá cao</a></li></button>
										<button name="pricehightolower" class="p-0 btn background-none"><li><a href="searchController.php?keyword=<?php echo $keyword ?>&acset=pricehightolowt" class="text-decoration-none">Giá cao đến giá thấp</a></li></button>
									</ul>
								</form>
							</div>
						</div>
