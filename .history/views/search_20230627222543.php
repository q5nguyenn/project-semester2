<?php 
	...
	if ($query_searchProduct) {
		...
		if (isset($_GET['keyword']) && isset($_GET['acset'])) {
			$searchProduct = "SELECT courses.*,COUNT(bill_course.course_id) as bill_course, users.name as users_name, users.thumbnail as users_thumbnail 
			FROM `courses`
			JOIN users ON courses.teacher_id = users.id
			JOIN bill_course ON bill_course.course_id = courses.id
			WHERE  courses.name LIKE '%".$keyword."%' ORDER BY teacher_id ASC ";
			$query_searchProduct = mysqli_query(openConnection(), $searchProduct);
