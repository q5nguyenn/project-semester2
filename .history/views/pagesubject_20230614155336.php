...
if(isset($_POST['buttonCourseHot'])){
	$getIdBreadcrumb = $_GET['breadcrumb'];
	$mergeCourse = "SELECT courses.*, departments.name as departments_name, faculties.name as faculties_name, users.name = users_name 
	FROM `courses`JOIN departments ON departments.id = courses.department_id AND departments.id = faculities.id AND faculties.id = '$getIdBreadcrumb'
	JOIN users ON 
}
?>
